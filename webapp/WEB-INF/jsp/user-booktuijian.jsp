		...
		<div class="templatemo-sidebar">
			...
			<nav class="templatemo-left-nav">
				...
				<li><a href="${pageContext.request.contextPath}/reader-zx?cert_id=${session}"><i class="i-font-normal"></i>个人信息</a></li>
					<li><a href="${pageContext.request.contextPath}/ciyun" class="active"><i
					...
					<li><a
						...
						><i class="i-font-normal"></i>图书查询</a></li>
				...
				<div class="row">
					<nav class="templatemo-top-nav col-lg-12 col-md-12">
						<ul class="text-uppercase">
							<li><a
								href="${pageContext.request.contextPath}/user-booktuijian">图书推荐</a></li>
			...
			<div class="templatemo-content-container">
				<div class="templatemo-flex-row flex-content-row">
					<table
						...
						<tbody>
							<form
								action="${pageContext.request.contextPath}/booktuijiannum"
								class="templatemo-login-form" method="post"
								enctype="multipart/form-data">
								<c:forEach items="${tuijians}" var="tuijians">
									<tr>

										<td>${tuijians.BOOK_TITLE}</td>
										<td>${tuijians.BOOK_AUTHOR}</td>
										<td>${tuijians.BOOK_PUBLISHER}</td>
										<td>${tuijians.BOOK_PUB_YEAR}</td>
										<td>${tuijians.BOOK_CALL_NO}</td>
										<c:set var="bookname" scope="request"
											value="${tuijians.BOOK_TITLE}" />
<!-- 										<td><a -->
...
<!-- 										</td> -->
                                        <c:if test="${tuijians.OPAC_FLAG==1 }">
                                        <td>可借阅</td>
                                        </c:if>
                                        <c:if test="${tuijians.OPAC_FLAG==0 }">
                                        <td>不可借阅</td>
                                        </c:if>
									</tr>
								</c:forEach>
									...
									<tr>
										...
										<td colspan="6" align="center">
											共${pagetotal+1}页,当前第${pagenum}页 <c:if test="${pagenum-3>0}">
												<a
													href="${pageContext.request.contextPath}/booktuijiannum?pagenum=${pagenum-3}">${pagenum-3}</a>
											</c:if> <c:if test="${pagenum-2>0}">
												<a
													href="${pageContext.request.contextPath}/booktuijiannum?pagenum=${pagenum-2}">${pagenum-2}</a>
											</c:if> <c:if test="${pagenum-1>0}">
												<a
													href="${pageContext.request.contextPath}/booktuijiannum?pagenum=${pagenum-1}">${pagenum-1}</a>
											</c:if> 
											<c:if test="${pagenum-1>0}">
											<a
											href="${pageContext.request.contextPath}/booktuijianup">上一页</a>&nbsp;
											</c:if>
											<c:if test="${pagenum<pagetotal+1}">
											<a
											href="${pageContext.request.contextPath}/booktuijiandn">下一页</a>&nbsp;
											</c:if>
