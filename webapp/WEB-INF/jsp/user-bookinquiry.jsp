...
<body>
	...
	<div class="templatemo-flex-row">
		<div class="templatemo-sidebar">
			...
			<nav class="templatemo-left-nav">
				<ul>
					...
					<li><a
						href="${pageContext.request.contextPath}/user-bookinquiry"
						class="active"><i class="i-font-normal"></i>图书查询</a></li>
		...
		<div class="templatemo-content col-1 light-gray-bg">
				...
				<div class="row">
					<nav class="templatemo-top-nav col-lg-12 col-md-12">
						<ul class="text-uppercase">
							<li><a
								href="${pageContext.request.contextPath}/user-bookinquiry">图书检索</a></li>
			...
			<div>&nbsp;</div>
			<form action="${pageContext.request.contextPath}/user-bookjiansuo">
					<div class="test" style="float:left;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;按书籍名字检索书籍： </div>
					<div>
					<input type="text" name="bookname"> 
					<input type="submit" value="检索书籍"name="jiansuo">
					</div>
					</form>
			<div class="templatemo-content-container">
				<div class="templatemo-flex-row flex-content-row">
					<table
						...
						<thead>
							<tr>
								<td align="center"><a href="" class="white-text templatemo-sort-by">书名
										<span class="caret"></span>
								</a></td>
								<td align="center"><a href="" class="white-text templatemo-sort-by">作者
										<span class="caret"></span>
								</a></td>
								<td align="center"><a href="" class="white-text templatemo-sort-by">出版社
										<span class="caret"></span>
								</a></td>
								<td align="center"><a href="" class="white-text templatemo-sort-by">出版时间
										<span class="caret"></span>
								</a></td>
								<td align="center"><a href="" class="white-text templatemo-sort-by">索书号
										<span class="caret"></span>
								</a></td>
								<td align="center">可否借阅</td>
							</tr>
						</thead>
						<tbody>
							<form
								action="${pageContext.request.contextPath}/user-bookinquirynum"
								...
								<c:forEach items="${bookinquiry}" var="bookinquiry">
									<tr>
										...
										<td>${bookinquiry.BOOK_AUTHOR}</td>
										<td>${bookinquiry.BOOK_PUBLISHER}</td>
										<td>${bookinquiry.BOOK_PUB_YEAR}</td>
										<td>${bookinquiry.BOOK_CALL_NO}</td>
										<c:set var="bookname" scope="request"
											value="${bookinquiry.BOOK_TITLE}" />
<!-- 										<td><a -->
<%-- 											href="${pageContext.request.contextPath}/user-booklend?bookname=<%=URLEncoder.encode(String.valueOf(request.getAttribute("bookname")),"UTF-8") %>">借阅</a> --%>
<!-- 										</td> -->
                                        <c:if test="${bookinquiry.OPAC_FLAG==1 }">
                                        <td>可借阅</td>
                                        </c:if>
                                        <c:if test="${bookinquiry.OPAC_FLAG==0 }">
                                        <td>不可借阅</td>
                                        </c:if>
									</tr>
								</c:forEach>
									...
									<tr>
										...
										<td colspan="6" align="center">
											共${pagetotal+1}页,当前第${pagenum}页 <c:if test="${pagenum-3>0}">
												<a
													href="${pageContext.request.contextPath}/user-bookinquirynum?pagenum=${pagenum-3}">${pagenum-3}</a>
											</c:if> <c:if test="${pagenum-2>0}">
												<a
													href="${pageContext.request.contextPath}/user-bookinquirynum?pagenum=${pagenum-2}">${pagenum-2}</a>
											</c:if> <c:if test="${pagenum-1>0}">
												<a
													href="${pageContext.request.contextPath}/user-bookinquirynum?pagenum=${pagenum-1}">${pagenum-1}</a>
											</c:if> <c:if test="${pagenum-1>0}">
											<a
											href="${pageContext.request.contextPath}/user-bookinquiryup">上一页</a>&nbsp;
											</c:if>
											<c:if test="${pagenum<pagetotal+1}">
											<a
											href="${pageContext.request.contextPath}/user-bookinquirydn">下一页</a>&nbsp;
											</c:if>
